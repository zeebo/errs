...
func makeKey(tag errs.Tag, k interface{}) key {
	return key{
		tag: tag,
		key: k,
	}
...
func Set(tag errs.Tag, key interface{}, value interface{}) {
	registry.Store(makeKey(tag, key), value)
}
...
func Get(err error, key interface{}) interface{} {
	for _, class := range errs.Tags(err) {
